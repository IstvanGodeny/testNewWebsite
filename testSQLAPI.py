...
app.config["SQLALCHEMY_ENGINE_OPTIONS"] = {"pool_recycle": 280}                     # PythonAnywhere only
...
@app.post('/upload')
def upload():
    f = request.files['upload']     # kulcs: 'upload'
    name = secure_filename(f.filename)
    path = os.path.join(app.static_folder, 'uploads', name)
    f.save(path)
    url = url_for('static', filename=f'uploads/'+name, _external=True)
